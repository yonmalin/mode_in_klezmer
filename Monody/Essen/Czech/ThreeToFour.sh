...
do
	if grep -q '\*[A-G]:' $file ; then
		solfa $file | grep -v '^=' | context -n 2 | sed 's/[0-9]//g' | sed 's/[{}]//g' | sed 's/^\.//' | rid -GLId | grep 'mi fa' >> Major ###mi to fa###
	elif grep -q '\*[a-g]:' $file ; then
		 solfa $file | grep -v '^=' | context -n 2 | sed 's/[0-9]//g' | sed 's/[{}]//g' | sed 's/^\.//' | rid -GLId | grep 'me fa' >> Minor ####me to fa##
