...
do
	if grep -q '\*[A-G]:' $file ; then
		solfa $file | grep -v '^=' | context -n 2 | sed 's/[0-9]//g' | sed 's/[{}]//g' | sed 's/^\.//' | rid -GLId | grep 're mi' >> Major   ##2 to 3###
	elif grep -q '\*[a-g]:' $file ; then
		 solfa $file | grep -v '^=' | context -n 2 | sed 's/[0-9]//g' | sed 's/[{}]//g' | sed 's/^\.//' | rid -GLId | grep 're me' >> Minor    ###re to me##
