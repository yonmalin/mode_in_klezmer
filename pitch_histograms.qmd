...
major_1 <- major_pitch_dur |> 
    filter(!grepl("[CDEF].*3", major_pitches)) |> 
    filter(!grepl("[789]", major_pitches)) |> 
    filter(!grepl("[A|B].*6", major_pitches)) |>
...
```{r major-percentagese, cho=F, warnings=FALSE, message=FALSE}
####percentages
x <-  pitches |> 
    pitch(simple = FALSE) |>
    humdrumR::tally() 
...
minor_1 <- minor_pitch_dur |> 
    filter(!grepl("[CDEF].*3", minor_pitches)) |> 
    filter(!grepl("[789]", minor_pitches)) |> 
    filter(!grepl("[A|B].*6", minor_pitches)) |>
...
meertens_major_in_G <- meertens_major |> mutate(kern(Token, simple = FALSE, transposeArgs = list(to = 'G:'))) 
 ...
 meertens_major_1 <- meertens_major_pitch_dur |> 
    filter(!grepl("[CDEF].*3", meertens_major_pitches)) |> 
    filter(!grepl("[789]", meertens_major_pitches)) |> 
    filter(!grepl("[A|B].*6", meertens_major_pitches)) |>
...
## Bag of Notes
```{r, major-bag-of-notes, echo=F, warnings=FALSE, message=FALSE}
...
ggplot(meertens_major_pitches, na.rm=TRUE, aes(x=Pitch, y=percentage)) +
    geom_bar(stat="identity") + theme_bw() +
    labs(y="percentage") +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 
```
...
meertens_minor_in_G <- meertens_minor |> mutate(kern(Token, simple = FALSE, transposeArgs = list(to = 'G:'))) 
 ...
 meertens_minor_1 <- meertens_minor_pitch_dur |> 
    filter(!grepl("[CDEF].*3", meertens_minor_pitches)) |> 
    filter(!grepl("[789]", meertens_minor_pitches)) |> 
    filter(!grepl("[A|B].*6", meertens_minor_pitches)) |>
...
ggplot(meertens_minor_pitches, na.rm=TRUE, aes(x=Pitch, y=percentage)) +
    geom_bar(stat="identity") + theme_bw() +
    labs(y="percentage") +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 
```
